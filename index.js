...
// ¡CORREGIDO AQUÍ! Ahora se importa como CitlaliJadiBot
import { FurinaJadiBot as CitlaliJadiBot } from './plugins/jadibot-serbot.js' 
...
if (connection == 'open') {
console.log(chalk.bold.green('\n❀ Citlali Conectada con éxito ❀'))
}
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
if (connection === 'close') {
if (reason === DisconnectReason.badSession) {
console.log(chalk.bold.cyanBright(`\n⚠︎ SIN CONEXIÓN: Borre la carpeta ${global.sessions} y escanee el código QR.`))
...
console.log(chalk.bold.yellowBright(`\n⚠︎ CONEXIÓN REEMPLAZADA: Se ha abierto otra nueva sesión. Cierre la sesión actual primero.`))
...
CitlaliJadiBot({pathCitlaliJadiBot: botPath, m: null, conn, args: '', usedPrefix: '/', command: 'serbot'}) 
}
}
}
}
...
await clearTmp()
console.log(chalk.bold.cyanBright(`\n◎ Multimedia: Archivos de la carpeta 'tmp' eliminados.`))}, 1000 * 60 * 4) // Cada 4 minutos

setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return
await purgeSession()
console.log(chalk.bold.cyanBright(`\n◎ ${global.sessions}: Sesiones no esenciales eliminadas.`))}, 1000 * 60 * 10) // Cada 10 minutos

setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return
await purgeSessionSB()}, 1000 * 60 * 10) // Cada 10 minutos

setInterval(async () => {
if (stopped === 'close' || !conn || !conn.user) return
await purgeOldFiles()
console.log(chalk.bold.cyanBright(`\n◎ Archivos: Archivos residuales eliminados.`))}, 1000 * 60 * 10) // Cada 10 minutos
